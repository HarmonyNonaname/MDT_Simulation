...
G4ThreadLocal G4Allocator<TubeHit>* TubeHitAllocator;
...
G4bool TubeHit::operator==(const TubeHit &/*right*/) const
{
  return false;
}
...
const std::map<G4String,G4AttDef>* TubeHit::GetAttDefs() const
{
  G4bool isNew;
  auto store = G4AttDefStore::GetInstance("TubeHit",isNew);
...
std::vector<G4AttValue>* TubeHit::CreateAttValues() const
{
  auto values = new std::vector<G4AttValue>;

  values
    ->push_back(G4AttValue("HitType","TubeHit",""));
  values
